...
export function KitchenOrderCard({ order, relativeTime, isUpdating, onUpdateStatus, onAcknowledgeAdditions }: KitchenOrderCardProps) {
  const statusInfo = ORDER_STATUS_MAP[order.status]
  const typeInfo = ORDER_TYPE_MAP[order.type]
  const nextAction = getNextAction(order)
  const timerBase = order.kitchenReceivedAt || order.createdAt
  const urgencyClasses = getUrgencyClasses(timerBase)
  const urgencyTextColor = getUrgencyTextColor(timerBase)
  const elapsedMins = getElapsedMinutes(timerBase)
    ...
    >
      <Card className={`border-2 ${urgencyClasses} bg-card overflow-hidden transition-all`}>
        ...
        <CardContent className="p-3 md:p-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.notes && (
                      <div className="mt-0.5 text-[10px] md:text-xs text-amber-400/80 italic">
                        ← {item.notes}
                      </div>
                    )}
                  </div>
          ...
          {order.items.some(i => i.preparationArea === 'KITCHEN' && (i.addedQuantity ?? 0) > 0) && onAcknowledgeAdditions && (
            <Button
              onClick={() => onAcknowledgeAdditions(order.id)}
              disabled={isUpdating}
              variant="outline"
              className="w-full gap-2 border-emerald-500/40 text-emerald-400 hover:bg-emerald-500/10 h-10 md:h-12 text-sm md:text-base font-bold rounded-lg mb-2"
            >
              {isUpdating ? (
