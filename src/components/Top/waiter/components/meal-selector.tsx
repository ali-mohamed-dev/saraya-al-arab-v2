...
import { CATEGORIES as HARDCODED_CATEGORIES, ORDER_TYPE_MAP, ORDER_STATUS_MAP } from '@/lib/saraya/constants'
...
interface MealSelectorProps {
  ...
  onSelectCustomer?: (result: CustomerResult) => void
...
export function MealSelector({
  ...
  setDeliveryAddress,
  existingTableOrder,
  allowedTypes = ['DINE_IN', 'TAKEAWAY', 'DELIVERY'],
  ...
  const emailLookupTimer = useRef<ReturnType<typeof setTimeout>>(undefined)
  useEffect(() => {
    if (customerEmail && customerEmail.includes('@') && onEmailLookup) {
      clearTimeout(emailLookupTimer.current)
      emailLookupTimer.current = setTimeout(() => onEmailLookup(customerEmail.trim()), 700)
    }
    return () => clearTimeout(emailLookupTimer.current)
  }, [customerEmail, onEmailLookup])
    ...
    <div className="flex-1 min-h-0 flex flex-col md:border-r border-border/50 overflow-hidden">
      ...
      <div className="p-4 border-b border-border/30 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {orderType === 'DINE_IN' && allowedTypes.includes('DINE_IN') && (
            <div>
          ...
          <div className={orderType === 'DINE_IN' && allowedTypes.includes('DINE_IN') ? '' : 'col-span-1'}>
            ...
            <div className="flex gap-2 mt-1">
              <Input
                ...
                dir="rtl"
              />
              {onSearchCustomer && customerName.length >= 2 && (
                <Button
                  type="button"
                  size="sm"
                  variant="outline"
                  onClick={() => onSearchCustomer(customerName)}
                  disabled={searchingCustomer}
                  className="shrink-0 h-9 border-[#D4AF37]/30 text-[#D4AF37] hover:bg-[#D4AF37]/10"
                >
                  {searchingCustomer ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Search className="h-3.5 w-3.5" />}
                </Button>
              )}
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2 mt-1">
              <Input
                ...
                dir="rtl"
              />
              {onSearchCustomer && customerPhone.length >= 6 && (
                <Button
                  type="button"
                  size="sm"
                  variant="outline"
                  onClick={() => onSearchCustomer(customerPhone)}
                  disabled={searchingCustomer}
                  className="shrink-0 h-9 border-[#D4AF37]/30 text-[#D4AF37] hover:bg-[#D4AF37]/10"
                >
                  {searchingCustomer ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Search className="h-3.5 w-3.5" />}
            ...
            {customerResults && customerResults.length > 0 && (
              <div className="mt-2 rounded-lg border border-[#D4AF37]/30 bg-muted/50 overflow-hidden">
                {customerResults.map((r, i) => (
                  <button
                    key={r.customerPhone + (r.customerName || '') + i}
                    type="button"
                    onClick={() => onSelectCustomer?.(r)}
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs text-right hover:bg-[#D4AF37]/10 transition-all border-b border-border/20 last:border-0"
                  >
                    <div className="flex-1 min-w-0">
                      <span className="font-bold text-[#D4AF37]">{r.customerName || 'بدون اسم'}</span>
                      <span className="text-muted-foreground mx-1">-</span>
                      <span dir="ltr" className="text-muted-foreground">{r.customerPhone}</span>
                      {r.deliveryAddress && (
                        <span className="block text-[10px] text-muted-foreground/60 truncate">{r.deliveryAddress}</span>
                      )}
                    </div>
                    <span className="text-[10px] text-green-400 font-bold shrink-0">تعبئة</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {orderType !== 'DINE_IN' && (
          <div>
            <Label className="text-xs text-muted-foreground">البريد الإلكتروني (مسجل)</Label>
            <div className="flex gap-2 mt-1">
              <Input
                type="email"
                value={customerEmail}
                ...
                onBlur={(e) => {
                  ...
                  if (val && val.includes('@')) onEmailLookup?.(val)
                }}
                placeholder="البريد الإلكتروني للعميل المسجل"
                className="bg-muted border-border/50 h-9"
                dir="rtl"
              />
            </div>
      ...
      {/* Search & filter */}
      <div className="px-4 pb-4 pt-2 border-b border-border/30 space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {['الكل', ...categories.filter((c) => c.value !== 'الكل').map((c) => c.value)].map((cat) => (
            <button
              key={cat}
