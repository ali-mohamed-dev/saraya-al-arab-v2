...
export function OrderDetailDialog({
  ...
  const totals = useMemo(() => {
    ...
    const serviceCharge = selectedOrder?.type === 'DINE_IN' ? Math.round(subtotal * SERVICE_CHARGE_RATE * 100) / 100 : 0
    const deliveryFee = selectedOrder?.type === 'DELIVERY' ? DELIVERY_FEE : 0
    const total = subtotal + serviceCharge + deliveryFee
    return { subtotal, serviceCharge, deliveryFee, total }
  ...
  const handleSaveOrderEdits = async () => {
    ...
    try {
      ...
      const requestBody: Record<string, unknown> = {
        ...
        tableNumber: editableOrder.tableNumber,
                ...
                <div className="rounded-lg border border-border/30 bg-muted/20 p-3">
                  ...
                  {selectedOrder.status !== 'DELIVERED' && selectedOrder.status !== 'CANCELLED' ? (
                    <Input
                      ...
                      min="1"
                      onChange={(e) => setEditableOrder((prev) => prev ? { ...prev, tableNumber: e.target.value || undefined } : prev)}
                      className="mt-1"
                      dir="rtl"
                    />
                  ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        {displayOrder.status === 'PENDING' ? ( // Use displayOrder
                          ...
                          <p className="text-sm font-semibold leading-tight break-words line-clamp-2">{item.mealTitleAr || item.mealTitle}</p>
                        )}
                          ...
                          <div className="mt-0.5 flex flex-wrap gap-1">
                            {item.addOns.map((addon, idx) => (
                              <Badge
                                  key={idx}
                                variant="outline"
                                className="border-orange-500/20 text-orange-400 text-[9px] px-1.5 py-0 max-w-[140px] truncate"
                              >
                      ...
                      <span className="text-sm font-bold text-[#D4AF37] flex-shrink-0 whitespace-nowrap">
            ...
            <div className="space-y-3 text-sm">
              ...
              {totals.deliveryFee > 0 && (
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span>{totals.deliveryFee.toFixed(2)} ج.م</span>
                </div>
              )}
              <Separator className="bg-border/30" />
