...
import { type Meal, type AddOn } from '@/lib/saraya/types'
...
export function OrderDetail({ meal, open, onClose }: OrderDetailProps) {
  ...
  const handleAddToCart = () => {
    ...
    toast({
      ...
      description: `${quantity > 1 ? quantity + '× ' : ''}${meal.titleAr}${selectedAddOns.length > 0 ? ` + ${selectedAddOns.length} إضافات` : ''}`,
    })
    setTimeout(() => {
    ...
    <Sheet open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      ...
      >
        <SheetHeader className="sr-only">
          <SheetTitle>{meal.titleAr || 'تفاصيل الطبق'}</SheetTitle>
        </SheetHeader>
